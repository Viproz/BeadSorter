...
// Max allowed color difference = thresholdFactor ∗ colorValue + offset
// (0,08/40) dark colors 0.02 + 10
float thresholdFactor = 0.04;
int offset = 40;

Servo servo;
AccelStepper stepper =
    AccelStepper(motorInterfaceType, STEPPER_STEP_PIN, STEPPER_DIR_PIN);
...
int nullScanValues[4] = {6618, 1860, 2282,
                         2116};  // adjust these if no calibration
...
int dynamicContainerArray[dynamicContainerArraySize] = {
    -1, 666, -1, -1, -1, 666, -1, -1, -1, 666, -1, -1, -1, 666, -1, -1};
...
void loop() {
    ...
    if (!nullScan()) {
        Serial.println("");
        Serial.print("Beads analyzed: ");
        Serial.println(beadCounter++);
        Serial.print("\tClear:");
        Serial.print(resultColor[0]);
        Serial.print("\tRed:");
        Serial.print(resultColor[1]);
        Serial.print("\tGreen:");
        Serial.print(resultColor[2]);
        Serial.print("\tBlue:");
        Serial.print(resultColor[3]);
        Serial.println("");
    ...
    // Serial.print("errorcounter: ");Serial.print(errorCounter);Serial.print("
    // maxErrorCounter:"); Serial.println(maxErrorCounter);
...
int getNextFreeArrayPlace() {
    int arrayCounter = 0;
    while ((storedColors[arrayCounter][0] > 0) and
...
void clearMedianColors() { memset(medianColors, 0, sizeof(medianColors)); }
...
void calcMedianAndStore() {
    ...
    Serial.print("I stored color to bank #");
    Serial.print(nextColorNo);
    Serial.println("");
}
...
boolean nullScan() {
    if ((resultColor[0] > (nullScanValues[0] - nullScanOffset) and
         resultColor[0] < (nullScanValues[0] + nullScanOffset)) and
        (resultColor[1] > (nullScanValues[1] - nullScanOffset) and
         resultColor[1] < (nullScanValues[1] + nullScanOffset)) and
        (resultColor[2] > (nullScanValues[2] - nullScanOffset) and
         resultColor[2] < (nullScanValues[2] + nullScanOffset)) and
        (resultColor[3] > (nullScanValues[3] - nullScanOffset) and
         resultColor[3] < (nullScanValues[3] + nullScanOffset))) {
        return true;
    }
    return false;
}
...
void setNullScanValues() {
    ...
    Serial.print(nullScanValues[0]);
    Serial.print(" ");
    Serial.print(nullScanValues[1]);
    Serial.print(" ");
    Serial.print(nullScanValues[2]);
    Serial.print(" ");
    Serial.print(nullScanValues[3]);
    Serial.println("");
}
...
void sortBeadToDynamicArray() {
    ...
    for (int i = 0; i < 16; i++) {
        ...
        //        Serial.print(i);Serial.print(":R:");
        //        Serial.print(upperLimit);Serial.println("");
        //        Serial.print(i);Serial.print(":R:");
        //        Serial.print(lowerLimit);Serial.println("");
        //        Serial.println(tempStoredColors[ i][1]);
        if (tempStoredColors[i][1] >= lowerLimit and
            tempStoredColors[i][1] <= upperLimit) {
            threshold = thresholdFactor * resultColor[2] + offset;
            upperLimit = resultColor[2] + threshold;
            lowerLimit = resultColor[2] - threshold;
            //
            //            Serial.print(i);Serial.print(":G:");
            //            Serial.print(threshold);Serial.println("");
            //            Serial.print(i);Serial.print(":G:");
            //            Serial.print(upperLimit);Serial.println("");
            //            Serial.print(i);Serial.print(":G:");
            ...
            if (tempStoredColors[i][2] >= lowerLimit and
                ...
                if (tempStoredColors[i][3] >= lowerLimit and
                    tempStoredColors[i][3] <= upperLimit) {
                    //          Serial.print("Color is #");
                    //          Serial.print(tempStoredColors[ i ][0]);
                    //          Serial.println("");
                    if (autoSort) {
                        Serial.print("Color is R:");
                        Serial.print(tempStoredColors[i][1]);
                        Serial.print(" G:");
                        Serial.print(tempStoredColors[i][2]);
                        Serial.print(" B:");
                        Serial.print(tempStoredColors[i][3]);
                        //            updateStoredColorCount(i);
                        //            updateDetectedColorFromTempStoredColor(i);
                    } else {
                        Serial.print("Color is #");
                        Serial.print(
                            getColorNameFromNo(tempStoredColors[i][0]));
                        Serial.print(". ");
                    }
                    // Serial.println(tempStoredColors[ i ][0]);

                    int containerNo = getContainerNo(tempStoredColors[i][0]);
                    // Serial.print("move stepper to container No:");
                    // Serial.println(containerNo);
    ...
    if (!found) {
        ...
        if (autoSort) {
            ...
            if (!allContainerFull()) {
                ...
                storeColor(autoColorCounter, resultColor[1], resultColor[2],
                           resultColor[3]);
                autoColorCounter++;
...
int getContainerNo(int colorIndex) {
    ...
    for (i; i < dynamicContainerArraySize; i++) {
        // Serial.print("looking for ");Serial.print(colorIndex);Serial.print("
        // at position ");Serial.println(i);
        if (colorIndex == dynamicContainerArray[i]) {
            Serial.print("Color found in container array: ");
            Serial.println(dynamicContainerArray[i]);
            containerNo = i;
            break;
        }
    }
...
int getNextContainerNo(int colorIndex) {
    ...
    Serial.print("Return Color Index: ");
...
//  Serial.println(""); Serial.print("Red before: "); Serial.print(storedColors[
//  i ][1]); Serial.print(" Color Counter: "); Serial.print(storedColors[ i
//  ][4]); storedColors[ i ][1] = ((storedColors[ i ][1] * (storedColors[ i ][4]
//  - 1)) + (resultColor[1])) / storedColors[ i ][4]; storedColors[ i ][2] =
//  ((storedColors[ i ][2] * (storedColors[ i ][4] - 1)) + (resultColor[2])) /
//  storedColors[ i ][4]; storedColors[ i ][3] = ((storedColors[ i ][3] *
//  (storedColors[ i ][4] - 1)) + (resultColor[3])) / storedColors[ i ][4];
//  Serial.println(""); Serial.print("Red after: "); Serial.print(storedColors[
//  i ][1]); Serial.println("");
//
//}
//
